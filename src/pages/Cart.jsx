...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      if (event.key === 'Escape' && !removing) setConfirmRemove(null);
    };
  ...
  const confirmDelete = async () => {
    ...
    setRemoving(true);
    try {
      const removed = await removeFromCart(confirmRemove.product, confirmRemove.size, confirmRemove.color);
  ...
  return (
    <div className="container cartflow">
                  ...
                  {g.items.map((i) => (
                    <div className="st-row" key={keyOf(i)}>
                      ...
                      <div className="st-actions">
                        <button
                          className="trash"
                          onClick={() => setConfirmRemove({ product: i.product, size: i.size, color: i.color, name: i.name, image: i.image, price: i.price })}
      ...
      {confirmRemove && (
        <>
          <div className="drawer-veil" onClick={() => !removing && setConfirmRemove(null)} />
          <div className="confirm-modal" role="dialog" aria-modal="true" aria-labelledby="remove-cart-title">
            <div className="confirm-icon"><Trash2 size={20} /></div>
            <div className="confirm-copy">
              <h3 id="remove-cart-title">Remove from cart?</h3>
              <p>{confirmRemove.name}</p>
              {(confirmRemove.color || confirmRemove.size) && (
                <small>{[confirmRemove.color && `Color: ${confirmRemove.color}`, confirmRemove.size && `Size: ${confirmRemove.size}`].filter(Boolean).join(' · ')}</small>
              )}
            </div>
            {confirmRemove.image && <img className="confirm-product-image" src={confirmRemove.image} alt="" />}
            <div className="confirm-actions">
              <button className="btn btn-outline" onClick={() => setConfirmRemove(null)} disabled={removing}>Keep item</button>
              <button className="btn btn-danger" onClick={confirmDelete} disabled={removing}>
