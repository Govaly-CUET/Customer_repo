...
export default function Listing() {
  ...
  useEffect(() => {
    api.get('/customer/categories').then(({ data }) => setCats(flattenCategories(data.data))).catch(() => {});
  }, []);

  useEffect(() => {
    setCategory(null);
    if (catSlug) api.get(`/customer/categories/${catSlug}`).then(({ data }) => setCategory(data.data)).catch(() => {});
  }, [catSlug]);

  const params = useMemo(() => {
    const p = { page, sort, limit: 24 };
    if (search) p.search = search;
    if (catSlug || selCats.length) p.category = catSlug || selCats.join(',');
    if (min) p.min = min;
    if (max) p.max = max;
    if (size) p.size = size;
    return p;
  }, [page, sort, search, catSlug, selCats.join(), min, max, size]);

  useEffect(() => {
    setLoading(true);
    api
      .get('/customer/products', { params })
      .then(({ data }) => setData({ ...data, items: normalizeProductList(data.items) }))
        ...
        <aside className={`filters${filtersOpen ? ' open' : ''}`}>
          ...
          <div className="fgroup">
            ...
            {parents.map((p, parentIndex) => (
              <div key={p.slug || p._id || `parent-${parentIndex}`} style={{ marginBottom: 4 }}>
                <Link
                  to={`/category/${p.slug}`}
                ...
                <div style={{ paddingLeft: 12, borderLeft: '2px solid var(--brand-100)', marginLeft: 2 }}>
                  {childrenOf(p.slug).map((c, childIndex) => (
                    <Link
                      key={c.slug || c._id || `${p.slug || parentIndex}-child-${childIndex}`}
                      to={`/category/${c.slug}`}
                      style={{
